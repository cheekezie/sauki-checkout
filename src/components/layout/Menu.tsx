...
import { FeesLogo, LogoIcon } from "../../assets";
...
const Menu = ({ collapsed }: prop) => {
  ...
  const visibleMenus = MenuLinks.filter((menu) => {
    ...
    if (menu.roles && menu.roles.length > 0) {
      return menu.roles.includes(role as any);
    }
    ...
    submenu: menu.submenu
      ? menu.submenu.filter((sub) => {
          ...
          if (sub.roles && sub.roles.length > 0) {
            return sub.roles.includes(role as any);
          }
    ...
    <>
      ...
      <div className="p-4 border-b border-slate-200/50 dark:border-slate-700/50">
        <div className="flex items-center space-x-3 justify-between">
          <div className="flex items-center relative h-10 w-24">
            <img
              src={LogoIcon}
              className={`absolute left-0 top-0 h-8 transition-all duration-300 ease-in-out ${
                collapsed
          ...
          return (
            <div key={index} className="">
              <button
                ...
                onClick={() =>
                  onMenuChanged(!!menu.submenu, menu.path, menu.id)
                }
                className={`${
                  isParentActive
              ...
              {collapsed && (
                ...
                >
                  <div className="px-4 py-3">
                    <h1 className="font-medium text-sm mb-3 border-b border-b-slate-100 pb-2">
                      {menu.label}
                    </h1>
                    {menu?.submenu?.map((sub, idx: number) => {
                      const isImplemented = implementedPaths.has(sub.path);
                      const to = isImplemented
                        ? buildDashboardPath(sub.path)
                        : "/#";
                      const isSubActive =
                        to !== "/#" &&
                      ...
                      return (
                        <button
                          ...
                          className={`w-full py-1 text-sm transition-all mt-2 duration-200 text-left block hover:bg-slate-100 cursor-pointer ${
                            isSubActive
                              ? "px-2 text-primary text-s border-l-2 border-l-primary"
                              : "text-slate-600"
                          }`}
                        >
                          {sub.label}
